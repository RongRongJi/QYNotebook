...
export default class Todolabel extends Component {
  ...
  render() {
    ...
    return (
      <View ref={(r) => this.label = r} style={[styles.container,{height:this.state.Height}]}>
            <TouchableOpacity style={{position:'absolute',left:20}}
              onPress={()=>this.onPressTodo()}>
              <Image style={styles.image} 
                source={this.state.ifPass?require('../main/images/passed.png'):require('../main/images/circle.png')}/>
            </TouchableOpacity>
            <View style={styles.textview} onLayout={this.changeView.bind(this)}>
                <Text style={this.state.ifPass?styles.donetext:styles.text}>{content}</Text>
                <Text>截止时间: {ddl}</Text>
            </View>
      </View>
    );
